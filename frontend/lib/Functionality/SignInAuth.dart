...
class AuthServiceLogin {
  final String baseURL = "http://192.168.43.217:8000/";

  LandTransferData? landTransferData;
  static LandTransferData? currentLandTransferData;
  late W3MService _w3mService;
  final loginUri = Uri.parse("http://192.168.43.217:8000/login/");
  final refreshTokenUri =
      Uri.parse("http://192.168.43.217:8000/api/token/refresh/");
  ...
  Future<String> resendConfirmationEmail(String email) async {
    try {
      ...
      var response = await http.post(
        Uri.parse("http://192.168.43.217:8000/resend_confirmation/"),
        headers: {"Content-Type": "application/x-www-form-urlencoded"},
  ...
  Future<void> logout() async {
    ...
    if (refreshToken != null) {
      var response = await http.post(
        Uri.parse(
            "http://192.168.43.217:8000/logout/"), // Adjust the URL to your backend's logout endpoint
        body: jsonEncode({"refresh": refreshToken}),
        headers: {"Content-Type": "application/json"},
      );
